...
const PackagingReport = ({ onClose, user, apiBase, onNotify }) => {
  ...
  const checklistItems = [
    { key: "paradasEmergencia", label: "Funcionamiento de paradas de emergencia" },
    { key: "integridadBoquillas", label: "Integridad de las boquillas correcta" },
    { key: "fechaLoteImpresos", label: "Fecha y lote impreso correctos" },
    { key: "fechaLoteLegibles", label: "Fecha y lote legibles y bien ubicados" },
    { key: "envasesCierran", label: "Los envases cierran correctamente" },
    { key: "etiquetaCorrecta", label: "Etiqueta correcta" },
    { key: "unidadesCaja", label: "La caja tiene las unidades correspondientes" },
  ];
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      paradasEmergencia: formData.paradasEmergencia,
      integridadBoquillas: formData.integridadBoquillas,
      fechaLoteImpresos: formData.fechaLoteImpresos,
      fechaLoteLegibles: formData.fechaLoteLegibles,
      envasesCierran: formData.envasesCierran,
      etiquetaCorrecta: formData.etiquetaCorrecta,
      unidadesCaja: formData.unidadesCaja,
    ...
    try {
      const response = await fetch(`${apiBase}/createPackagingReport`, {
        method: "POST",
        headers: {
      ...
      if (!response.ok && response.status === 404) {
        throw new Error("La función createPackagingReport no está disponible. Por favor, despliega las funciones actualizadas.");
      }
      ...
      notify("success", "Informe de Envasado enviado correctamente. El informe ha sido guardado en la base de datos.");
      onClose();
    } catch (error) {
